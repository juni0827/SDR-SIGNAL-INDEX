...
function Shell({path, title, eyebrow, children}: {path:string;title:string;eyebrow:string;children:React.ReactNode}) {
  ...
  useEffect(() => {
    ...
    const initialOnlineState = window.setTimeout(() => setOnline(navigator.onLine), 0);
    window.addEventListener("keydown", key);window.addEventListener("online", on);window.addEventListener("offline", off);
    return () => {window.clearTimeout(initialOnlineState);window.removeEventListener("keydown", key);window.removeEventListener("online", on);window.removeEventListener("offline", off);};
  }, [command,setCommand]);
...
function Waveform(){return <div className="waveform">{Array.from({length:140},(_,i)=><i key={i} style={{height:`${(10+Math.abs(Math.sin(i*.43))*78).toFixed(4)}%`}}/>)}<span/><em>VOICE · 00:16.2–00:48.9</em></div>;}
function AudioReview({segmentId}:{segmentId?:string}){
  const audioRef=useRef<HTMLAudioElement>(null);const audioContext=useRef<AudioContext|null>(null);const gainNode=useRef<GainNode|null>(null);
  const segmentQuery=useQuery({queryKey:["segment",segmentId],enabled:Boolean(segmentId),queryFn:()=>api<Envelope<Record<string,unknown>>>(`/segments/${encodeURIComponent(segmentId!)}`)});
  const recordingId=String(segmentQuery.data?.data.recording_id??"");
  const mediaQuery=useQuery({queryKey:["recording-media",recordingId],enabled:Boolean(recordingId),queryFn:()=>api<Envelope<{original_url:string;processed_url:string|null;preview_url:string|null}>>(`/recordings/${encodeURIComponent(recordingId)}/media`)});
  const [variant,setVariant]=useState<"processed"|"original"|"preview">("processed");const [playing,setPlaying]=useState(false);const [text,setText]=useState("Kilo seven two, message follows. Two eight one, four six, nine nine two. End.");const [saved,setSaved]=useState(false);
  const source=variant==="original"?mediaQuery.data?.data.original_url:variant==="preview"?mediaQuery.data?.data.preview_url:mediaQuery.data?.data.processed_url;
  const toggle=async()=>{const audio=audioRef.current;if(!audio)return;if(audio.paused){await audio.play();setPlaying(true);}else{audio.pause();setPlaying(false);}};
  const gain=(value:number)=>{const audio=audioRef.current;if(!audio)return;try{if(!audioContext.current){const context=new AudioContext();const node=context.createMediaElementSource(audio);const control=context.createGain();node.connect(control).connect(context.destination);audioContext.current=context;gainNode.current=control;}if(gainNode.current)gainNode.current.gain.value=value;}catch(error){console.warn("web_audio_gain_fallback",error);audio.volume=Math.min(1,value);}};
  const save=async()=>{if(segmentId)await api(`/segments/${encodeURIComponent(segmentId)}/transcripts`,{method:"POST",body:JSON.stringify({text,language:"en",mark_preferred:true})});setSaved(true);};
  return <div className="review" data-testid="audio-review"><section className="panel audio"><header><div><span className="kicker">ORIGINAL PRESERVED · {recordingId.slice(0,8)||"R-0021"}</span><h2>4.625 MHz · USB</h2></div><Layer kind="observed"/></header><div className="tabs"><button aria-pressed={variant==="processed"} onClick={()=>setVariant("processed")}>Processed · voice 300–3,000 Hz</button><button aria-pressed={variant==="original"} onClick={()=>setVariant("original")}>Original</button><button aria-pressed={variant==="preview"} onClick={()=>setVariant("preview")}>Noise preview</button></div><audio ref={audioRef} src={source??undefined} preload="metadata" onEnded={()=>setPlaying(false)} aria-label="Recording audio"/><Waveform/><div className="spectrogram">{Array.from({length:100},(_,i)=><i key={i} style={{left:`${i}%`,top:`${(i*37)%90}%`}}/>)}</div><footer className="transport"><button onClick={()=>void toggle()} aria-label={playing?"Pause audio":"Play audio"}>{playing?<Pause/>:<Play/>}</button><b>Signed range-streamed audio</b>{[.75,1,1.25].map(rate=><button key={rate} onClick={()=>{if(audioRef.current)audioRef.current.playbackRate=rate;}}>{rate}×</button>)}<label>Gain<input type="range" min=".1" max="2" step=".1" defaultValue="1" onChange={event=>gain(Number(event.target.value))}/></label><button onClick={()=>{if(audioRef.current)audioRef.current.loop=!audioRef.current.loop;}}>Loop</button></footer><div className="segment-actions"><button>Split</button><button>Merge</button><button>Rerun VAD</button><b>3 voice · 1 noise</b></div></section><section className="panel transcript"><header><div><span className="kicker">TRANSCRIPT CANDIDATES</span><h2>Segment {segmentId?.slice(0,8)??"SG-210"}</h2></div><b>84%</b></header><div className="tabs"><button>Preferred · corrected</button><button>ASR · large-v3</button></div><Layer kind="corrected"/><textarea data-testid="transcript-editor" aria-label="Transcript correction" value={text} onChange={e=>setText(e.target.value)}/><div className="entities"><span>EXTRACTED</span><button>CALLSIGN · KILO 72</button><button>NUMBER · 281</button><button>NUMBER · 46</button><button>NUMBER · 992</button></div><label>Annotation<textarea placeholder="Evidence-linked note…"/></label><footer><button>Save alternative</button><button className="primary" data-testid="save-transcript" onClick={()=>void save()}>Save corrected & preferred</button></footer>{saved?<p role="status">Correction saved; machine candidate preserved.</p>:null}</section></div>;
}
