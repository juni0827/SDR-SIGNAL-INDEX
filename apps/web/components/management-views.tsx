...
export function SourcesView({ id }: { id?: string }) {
  ...
  const profiles = useQuery({ queryKey: ["source-profiles"], enabled: !id, queryFn: () => api<Envelope<Row[]>>("/sources/profiles") });
  ...
  const installProfile = useMutation({
    mutationFn: (profileId: string) => api<Envelope<Row>>(`/sources/profiles/${encodeURIComponent(profileId)}/install`, { method: "POST", body: "{}" }),
    onSuccess: () => {
      void client.invalidateQueries({ queryKey: ["sources"] });
      void client.invalidateQueries({ queryKey: ["automation-status"] });
    },
  ...
  if (id) {
    ...
    const remote = ["rss_atom", "generic_html_table", "websdr_directory", "kiwisdr_directory", "priyom_schedule"].includes(String(row?.adapter_type));
  ...
  return (
    <div className="split">
      <section className="panel"><h2>Connected catalogue sources</h2><div className="source-profiles" aria-label="Public source profiles"><DataState loading={profiles.isLoading} error={profiles.error} empty={(profiles.data?.data.length ?? 0) === 0}>{(profiles.data?.data ?? []).map(profile => { const profileConfig = (profile.config as Row | undefined) ?? {}; const permissionRequired = Boolean(profileConfig.requires_terms_approval); return <article className="queue" key={String(profile.id)}><Layer kind="observed"/><b>{String(profile.name)}</b><small>{String(profile.description)}</small><button className="primary" onClick={() => installProfile.mutate(String(profile.id))} disabled={installProfile.isPending}>{permissionRequired ? "Install paused profile" : "Connect & schedule"}</button></article>; })}</DataState>{installProfile.error ? <p role="alert">{installProfile.error.message}</p> : null}</div><h2>Registered sources</h2><DataState loading={list.isLoading} error={list.error} empty={(list.data?.data.length ?? 0) === 0}><div className="cards">{(list.data?.data ?? []).map(row => <Link href={`/sources/${String(row.id)}`} key={String(row.id)}><Layer kind="observed"/><h2>{String(row.name)}</h2><p>{String(row.adapter_type)} · {row.enabled ? "enabled" : "disabled"}</p></Link>)}</div></DataState></section>
      <aside className="panel inbox-form"><h2>Register source</h2><form onSubmit={submit}><label>Name<input name="name" required/></label><label>Adapter<select name="adapter_type" value={adapterType} onChange={event => setAdapterType(event.target.value)}><option value="rss_atom">RSS/Atom</option><option value="generic_html_table">HTML table</option><option value="websdr_directory">WebSDR directory</option><option value="kiwisdr_directory">KiwiSDR directory</option><option value="priyom_schedule">Number-station schedule</option><option value="user_defined_static">Static (manual)</option></select></label><label>Record type<select name="record_type"><option>EVENT</option><option>FREQUENCY</option><option>RECEIVER</option></select></label><label>URL<input name="base_url" type="url" required={adapterType !== "user_defined_static"}/></label><label>Poll interval seconds<input name="interval_sec" type="number" min="300" max="604800" defaultValue="3600" required disabled={adapterType === "user_defined_static"}/></label><label><input name="enabled" type="checkbox" defaultChecked={adapterType !== "user_defined_static"} disabled={adapterType === "user_defined_static"}/> Start unattended collection after registration</label><button className="primary">Register source</button>{create.error ? <p role="alert">{create.error.message}</p> : null}</form><p className="policy">Profiles run in the worker after the browser closes. Every fetch honors the registered hostname, rate limit, conditional headers, and robots policy.</p></aside>
    </div>
  );
}
